...
Instance of the class have a `msg` property to retrieve the value at
...
subclass of `Kwexception`, the method returns the same exception instance, but
...
`msg` as a Python format-string, as documented above.
...
update or setdefault: `NEW_UPDATE`**. When given an instance of the relevant
`Kwexception` subclass, the classmethod `new()` uses the keyword parameters to
augment the original exception's `params` dict in the manner of
`dict.update()`. If `NEW_UPDATE` is set to false, the `params` dict is
augmented in the manner of `dict.setdefault`.
...
**New exceptions from original exceptions: key names for contextual
information: `CONTEXT_ERROR` and `CONTEXT_ARGS`**. Modify as needed.
